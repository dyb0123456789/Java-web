...
public class AddSubOneServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * String iId = request.getParameter("iId"); String uId =
		 * (String)request.getSession().getAttribute("uId");
		 * 
		 * String sub = request.getParameter("sub"); String add =
		 * request.getParameter("add"); cartDao cartdao = new cartDao(); int num =
		 * cartdao.itOfNum(uId,iId);
		 * 
		 * if(sub!=null) { //将uId和iId对应的num-1 //如果num为0的话就移除这个商品 if(num>1) {
		 * cartdao.setNumOfIt(uId, iId, (num-1)+"");
		 * 
		 * }else { //移除这个商品 cartdao.removeItem(uId, iId); }
		 * 
		 * }
		 * 
		 * if(add!=null) { //num+1 cartdao.setNumOfIt(uId, iId, (num+1)+""); }
		 * 
		 * //返回cartShopping response.sendRedirect("/shopping/cartShopping.jsp");
		 */
